...
class ImageMaker:
    ...
    def _print_text(self):
        #  Здесь я не хочу использовать цикл, потому что, если структура входящих данных изменится
        #  может быть сложно сдвинуть конкретное место так, чтобы не поплыла вся карточка
        #  ну и в целом код будет, конечно, короче, но поддерживать его станет труднее
