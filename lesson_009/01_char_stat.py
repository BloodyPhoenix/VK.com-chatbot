...
class InOutBlock:
# TODO Честно говоря, я так и не поняла, зачем здесь делать реализацию именно через классы.
# TODO Какая разница, дописывать функцию в модуль или метод к классу, если, по сути, нам нужно провести
# TODO операцию над данными? Мне кажется, эта задача вполне допускает использование функционального программирования
# TODO вместо обязательного ООП. От класса, конечно, можно наследоваться, но я правда не понимаю, зачем это здесь.
# TODO Вообще, одна из причин, по которой я начала учить Python - это поддержка не только ООП, но и функционального
# TODO программирования.
    def __enter__(self):
        return self
